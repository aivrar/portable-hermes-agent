...
def _load_cli_config() -> dict:
    """Load the same normalized, profile-aware configuration as Hermes CLI."""
    try:
        from hermes_cli.config import load_config

        return load_config()
    except Exception:
        # Guided mode must still open when the user's YAML is unreadable.
        return {
...
class AgentBridge:
    ...
    def __init__(self, root, on_response=None, on_tool_call=None,
        ...
        # Mark this as an interactive frontend. Dangerous actions are routed to
        # the GUI approval dialog on the worker thread; never enable YOLO mode.
    ...
    def get_model(self) -> str:
        if not hasattr(self, '_selected_model') or not self._selected_model:
            ...
            if isinstance(model_config, str):
                ...
                configured = model_config.get("default") or model_config.get("model")
                if isinstance(configured, dict):
                    from hermes_cli.config import split_model_config_default

                    configured, _provider = split_model_config_default(configured)
            self._selected_model = str(configured or "google/gemini-2.5-flash")
    ...
    def _create_agent(self):
        ...
        if self._active_provider == "local" and self._lm_studio_base_url:
            ...
            # GUI follows upstream auth, endpoint, and API-mode changes.
        ...
        # Use the same per-model reasoning and fallback resolution as the CLI.
        from hermes_constants import resolve_reasoning_config
        from hermes_cli.fallback_config import get_fallback_chain
        from hermes_cli.config import resolve_turn_limit

        reasoning_config = resolve_reasoning_config(self.config, model)

        fallback = get_fallback_chain(self.config)
...
The terminal tool uses **Git Bash** with working directory `{proj_bash}`.
...
- **NEVER** hard-code install paths — this folder may be moved to another drive or directory
...
- For write_file tool, **ALWAYS write temp scripts to `workspace/`** folder: `{PROJECT_ROOT / 'workspace' / 'myscript.py'}`
...
""")
        ...
        self.agent = AIAgent(
            ...
            max_iterations=resolve_turn_limit(agent_config.get("max_turns", 90)),
            reasoning_config=reasoning_config,
            fallback_model=fallback,
            # Provider routing
            providers_allowed=routing.get("only", routing.get("allowed")),
            providers_ignored=routing.get("ignore", routing.get("ignored")),
    ...
    def _approval_callback(self, command, description="", **kwargs):
        """Called when agent wants to run a dangerous command. Blocks until user approves."""
        self._post(self.on_approval, command, description)
        try:
            return "once" if self._approval_queue.get(timeout=60) else "deny"
        except queue.Empty:
            return "timeout"
    ...
    def _is_model_configured(self) -> bool:
        ...
        if self._active_provider == "local":
            return self._resolve_lm_studio_url() is not None
        try:
            from hermes_cli.runtime_provider import resolve_runtime_provider

            runtime = resolve_runtime_provider(target_model=self.get_model())
            return bool(runtime.get("provider") and runtime.get("api_key"))
        except Exception:
            return False
    ...
    def _run_agent(self, message: str, image_paths: list = None):
        ...
        # Approval callbacks are thread-local in current Hermes. Install the
        # bridge in the worker that actually executes tools, then always clear
        # it before that thread exits.
    ...
    def close(self):
        """Release resources owned by the GUI bridge."""
        self.interrupt()
